...
GPIO.setup(mq3Pin, GPIO.IN, pull_up_down=GPIO.PUD_DOWN)

# Function that lets the user know if gas gas been detected
def action(pin):
	print("Gas Detected!")
	return
...
GPIO.add_event_detect(mq3Pin, GPIO.RISING)

# After detected, the following line will call the action function
GPIO.add_event_callback(mq3Pin, action)
